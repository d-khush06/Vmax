...
export default function ManageAccountModal({ onClose }: ManageAccountModalProps) {
  ...
  return (
    <motion.div
      ...
      className="relative w-full max-w-4xl h-[600px] overflow-hidden rounded-3xl bg-white/10 backdrop-blur-[60px] border border-white/20 shadow-2xl flex"
    >
      {/* Sidebar */}
        ...
        <div className="flex flex-col gap-1">
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors font-medium text-sm ${
              activeTab === 'profile' ? 'bg-blue-500/20 text-blue-400 shadow-sm' : 'text-gray-400 hover:text-gray-200 hover:bg-white/5'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors font-medium text-sm ${
              activeTab === 'security' ? 'bg-blue-500/20 text-blue-400 shadow-sm' : 'text-gray-400 hover:text-gray-200 hover:bg-white/5'
            }`}
          >
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
          {activeTab === 'profile' && (
            <div className="max-w-xl">
              <h3 className="text-lg font-semibold text-white mb-6 border-b border-white/10 pb-4">Profile Details</h3>
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-bold text-white/50 uppercase tracking-wider mb-2 block">First Name</label>
                    <input
                      type="text"
                      value={firstName}
                      onChange={(e) => setFirstName(e.target.value)}
                      className="w-full bg-black/20 border border-white/10 rounded-2xl px-4 py-3 text-[15px] text-white focus:outline-none focus:border-blue-500/50 transition-all"
                    />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-white/50 uppercase tracking-wider mb-2 block">Last Name</label>
                    <input
                      type="text"
                      value={lastName}
                ...
                <div>
                  <label className="text-xs font-bold text-white/50 uppercase tracking-wider mb-2 block">Username</label>
                  <input
                    type="text"
                    value={username}
                ...
                <div>
                  <label className="text-xs font-bold text-white/50 uppercase tracking-wider mb-2 block">Primary Email</label>
                  <input
                    type="text"
                    value={user?.primaryEmailAddress?.emailAddress || ''}
                    disabled
                    className="w-full bg-black/30 border border-white/5 rounded-2xl px-4 py-3 text-[15px] text-white/50 cursor-not-allowed"
                  />
                  <p className="text-xs text-white/40 mt-2 font-medium">Email addresses are managed via your primary authentication provider.</p>
                </div>
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-white/10 flex items-center justify-end gap-3">
                ...
                <button
                  ...
                  disabled={isSaving}
                  className="px-6 py-3 rounded-xl bg-blue-500 hover:bg-blue-600 text-white text-sm font-semibold transition-all shadow-lg shadow-blue-500/25 disabled:opacity-50 flex items-center gap-2"
                >
          ...
          {activeTab === 'security' && (
            <div className="max-w-xl">
              <h3 className="text-lg font-semibold text-white mb-6 border-b border-white/10 pb-4">Security Settings</h3>
              
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center">
                <Shield size={32} className="text-white/50 mx-auto mb-3" />
